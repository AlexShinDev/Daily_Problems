...
class Superhero
  ...
  attr_writer :hitpoints, :alive, :has_special_tool, :attack
...
#need to create method to check whether someone si alive 
  def hit(hero)
    if hero.name == "Batman"
      opp_attack = attack
    else
    ...
    p "opp_attack = #{opp_attack}"
    p hero.hitpoints = hero.hitpoints - opp_attack
    # @hero.hitpoints = hero.hitpoints - hero.attack
    hero.alive = false if hero.hitpoints < 1
  end
...
p superman = Superhero.new({name: "Superman", hitpoints: 45, attack: 3})
p batman = Superhero.new({name: "Batman", hitpoints: 30, attack: 3})
